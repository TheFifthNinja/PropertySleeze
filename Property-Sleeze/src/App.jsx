...
export default function Example() {
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 min-h-screen`}>
      <header className="absolute inset-x-0 top-0 z-50">
        <nav aria-label="Global" className={`flex items-center justify-between p-4 lg:px-8 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className={`text-sm font-semibold leading-6 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {item.name}
              </a>
            ))}
          </div>
          <div className="hidden lg:flex lg:flex-1 lg:justify-end">
            <a href="#" className={`text-sm font-semibold leading-6 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
        ...
        <Dialog open={mobileMenuOpen} onClose={setMobileMenuOpen} className="lg:hidden">
          ...
          <DialogPanel className={`fixed inset-y-0 right-0 z-50 w-full overflow-y-auto px-6 py-6 sm:max-w-sm sm:ring-1 ${darkMode ? 'bg-gray-800 ring-gray-900/10' : 'bg-white ring-gray-900/10'}`}>
      ...
      <main className={`relative isolate px-6 pt-14 lg:px-8 ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-white text-gray-900'}`}>
        <div
          aria-hidden="true"
          className={`absolute inset-x-0 -top-40 -z-10 transform-gpu overflow-hidden blur-3xl sm:-top-80 ${darkMode ? 'bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30' : 'bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30'}`}
        >
          <div
            style={{
              clipPath:
        ...
        <div
          aria-hidden="true"
          className={`absolute inset-x-0 top-[calc(100%-13rem)] -z-10 transform-gpu overflow-hidden blur-3xl sm:top-[calc(100%-30rem)] ${darkMode ? 'bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30' : 'bg-gradient-to-tr from-[#ff80b5] to-[#9089fc] opacity-30'}`}
        >
          <div
            style={{
              clipPath:
